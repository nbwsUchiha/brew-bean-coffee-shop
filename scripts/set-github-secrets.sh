#!/usr/bin/env bash
# Load .env.local and worker/.dev.vars, then push secrets to GitHub Actions.
set -euo pipefail
...
if [[ ! -f "$ROOT/.env.local" ]]; then
  echo "Missing .env.local — copy from .env.example and fill in values first."
  exit 1
fi
...
STRIPE_SUCCESS_URL="${STRIPE_SUCCESS_URL:-https://${CLOUDFLARE_PAGES_PROJECT_NAME}.pages.dev/success}"
STRIPE_CANCEL_URL="${STRIPE_CANCEL_URL:-https://${CLOUDFLARE_PAGES_PROJECT_NAME}.pages.dev/cancel}"
